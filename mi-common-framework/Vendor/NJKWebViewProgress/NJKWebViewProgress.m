...
NSString * const NJKCompleteRPCURLPath = @"/njkwebviewprogressproxy/complete";
...
{
    if ([request.URL.path isEqualToString:NJKCompleteRPCURLPath]) {
...
- (void)webViewDidFinishLoad:(UIWebView *)webView
{
    ...
    if (interactive) {
        _interactive = YES;
        NSString *waitForCompleteJS = [NSString stringWithFormat:@"window.addEventListener('load',function() { var iframe = document.createElement('iframe'); iframe.style.display = 'none'; iframe.src = '%@://%@%@'; document.body.appendChild(iframe);  }, false);", webView.request.mainDocumentURL.scheme, webView.request.mainDocumentURL.host, NJKCompleteRPCURLPath];
